...
completes \u2014 do NOT sleep, poll, or proactively check on its progress. Continue \
...
as the `to` field. The agent resumes with its full context preserved. Each Agent \
...
class AgentTool(BaseTool):
    ...
    input_schema = {
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    "Optional model override for this agent. Takes precedence over the "
                    "agent definition's model frontmatter. If omitted, uses the agent "
                    "definition's model, or inherits from the parent."
                ),
            },
            "run_in_background": {
                "type": "boolean",
            ...
            "isolation": {
                ...
                "enum": ["worktree"],
                "description": (
                    'Isolation mode. "worktree" creates a temporary git worktree so '
    ...
    async def call(self, input_data: dict, context: ToolContext) -> ToolResult:
        ...
        try:
            result = await engine.run_turn(prompt_text)
