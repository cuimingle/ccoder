"""BashTool — execute shell commands with timeout and background support."""
...
destructive actions is unhelpful and can result in lost work, so it's best to ONLY run \
these commands when given direct instructions \
...
previous changes. Instead, after hook failure, fix the issue, re-stage, and create a NEW commit
...
only commit when explicitly asked, otherwise the user will feel that you are being too proactive
...
class BashTool(BaseTool):
    ...
    def get_activity_description(self, input: dict | None = None) -> str | None:
        if input and "description" in input:
    ...
    async def call(self, input: dict, context: ToolContext) -> ToolResult:
        ...
        timeout_ms = float(input.get("timeout", DEFAULT_TIMEOUT_MS))
        timeout_ms = min(timeout_ms, MAX_TIMEOUT_MS)
        timeout_s = timeout_ms / 1000.0
        run_bg = input.get("run_in_background", False)
        ...
        try:
            ...
            if run_bg:
                # Return immediately, the background task will be tracked separately
                return ToolResult(
            ...
            if proc.returncode != 0:
                return ToolResult(
                    content=combined or f"Command exited with code {proc.returncode}",
                    is_error=True,
                )
